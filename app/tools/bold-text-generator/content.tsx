        ...
        <div className="mx-auto max-w-4xl px-6 lg:px-12 space-y-5">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {([
                ...
                <button key={s.value} onClick={() => { setStyle(s.value); if (input) makeBold(input, s.value); }} className={`rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${style === s.value ? "border-violet-300 bg-violet-50 text-violet-700" : "border-slate-200 bg-white text-slate-600 hover:border-violet-200"}`}>{s.label}</button>
              ))}
            </div>
          </div>

          <div className="flex items-center gap-3">
            <button onClick={() => makeBold()} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-violet-600 px-6 text-sm font-semibold text-white shadow-lg shadow-violet-600/25 transition-colors hover:bg-violet-700"><Bold className="h-4 w-4 flex-shrink-0" /><span>Make Bold</span></button>
          ...
          <div>
            ...
            <div className="relative">
              <textarea value={output} readOnly placeholder="Bold text will appear here..." rows={5} className="w-full rounded-xl border border-slate-300 bg-slate-50 px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 font-mono" />
              {output && (
